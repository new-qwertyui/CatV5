		...
		Function = function(callback)
			if callback then
                hooksignal(replicatedStorage.Remotes.ToolRemotes.Knockback.OnClientEvent, function(old, attachment, value)
                    ...
                    return old(attachment, attachment, Vector3.new(horizontals[1], vertical, horizontals[2]))
                end)
			else
                ...
                repeat
                    if entitylib.isAlive then
                        ...
                        if plr and sword then
                            ...
                            replicatedStorage.Remotes.ToolRemotes.OnSwordHit:FireServer(sword, CFrame.lookAt(entitylib.character.RootPart.Position, Vector3.new(vec.X, entitylib.character.RootPart.Position.Y + 0.001, vec.Z)).LookVector, plr.Player.Character, workspace:GetServerTimeNow(), '9d')
